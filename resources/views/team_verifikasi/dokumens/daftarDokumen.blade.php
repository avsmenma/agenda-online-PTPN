...
@section('content')
  ...
  <script>
    ...
    (function () {
      ...
      // Sort toggle function for table columns
      function toggleSortImpl(column) {
      ...
      window.toggleSort = toggleSortImpl;
  ...
  <style>
    ...
    .data-table tbody tr.locked-row::after {
      display: none;
    ...
    .data-table tbody tr.locked-row .col-status .badge-locked {
      position: relative;
      z-index: 1;
    ...
    .data-table tbody tr.locked-row td {
      position: relative;
      z-index: 1;
    }

    .data-table tbody tr.locked-row .btn-action {
      position: relative;
      z-index: 1;
    }

    .data-table tbody tr.locked-row .badge-status {
      position: relative;
      z-index: 1;
    }

    .data-table tbody tr.locked-row .col-status .badge-locked::before {
      display: none;
      /* Removed warning icon animation */
    }

    /* Enhanced locked button states - Simplified */
    .data-table tbody tr.locked-row .btn-action.locked {
      position: relative;
      overflow: hidden;
      opacity: 0.6;
    }

    .data-table tbody tr.locked-row .btn-action.locked::before {
      display: none;
      /* Removed shimmer animation */
    }

    .data-table tbody tr.locked-row .btn-action.locked:hover {
      opacity: 0.7;
      cursor: not-allowed;
    }

    /* Lock hover tooltip enhancement */
    .data-table tbody tr.locked-row:hover .btn-action.locked {
      cursor: not-allowed;
    }
    ...
    @media (prefers-reduced-motion: reduce) {
      ...
      .data-table tbody tr.locked-row .col-status .badge-locked,
      .data-table tbody tr.locked-row .btn-action.locked::before,
      .data-table tbody tr.locked-row .deadline-empty {
        animation: none;
      }
    }
  ...
  <div class="vsummary-strip" aria-label="Ringkasan dokumen verifikasi">
    <a href="{{ route('documents.verifikasi.index') }}" class="vsummary-item vsummary-item--total">
      ...
      <div class="vsummary-meta">
        ...
        <div class="vsummary-value" data-counter="{{ $totalDokumenAgenda ?? 0 }}" data-duration="1400">0</div>
        <div class="vsummary-sub" data-counter-rupiah="{{ $totalNilaiRupiah ?? 0 }}" data-duration="1600">Rp 0</div>
      </div>
    </a>

    <a href="{{ route('documents.verifikasi.index') }}" class="vsummary-item">
      <div class="vsummary-icon"><i class="fas fa-folder-open"></i></div>
      <div class="vsummary-meta">
        <div class="vsummary-label">Verifikasi</div>
        <div class="vsummary-value" data-counter="{{ $totalDokumenVerifikasi ?? 0 }}" data-duration="1200">0</div>
        <div class="vsummary-sub">di verifikasi</div>
      </div>
    </a>

    <a href="{{ route('documents.verifikasi.index', ['status' => 'terkirim']) }}" class="vsummary-item">
      <div class="vsummary-icon"><i class="fas fa-paper-plane"></i></div>
      <div class="vsummary-meta">
        <div class="vsummary-label">Terkirim</div>
        <div class="vsummary-value" data-counter="{{ $totalTerkirim ?? 0 }}" data-duration="1200">0</div>
    ...
    <a href="{{ route('documents.verifikasi.index', array_merge(request()->except('keterlambatan', 'page'), $filterKeterlambatan === 'aman' ? [] : ['keterlambatan' => 'aman'])) }}"
      class="vsummary-item vsummary-item--safe {{ $filterKeterlambatan === 'aman' ? 'vsummary-item--active' : '' }}">
      <div class="vsummary-icon"><i class="fas fa-shield-alt"></i></div>
      <div class="vsummary-meta">
        <div class="vsummary-label">Aman</div>
        <div class="vsummary-value" data-counter="{{ $dokumenLessThan24h ?? 0 }}" data-duration="1000">0</div>
    ...
    <a href="{{ route('documents.verifikasi.index', array_merge(request()->except('keterlambatan', 'page'), $filterKeterlambatan === 'peringatan' ? [] : ['keterlambatan' => 'peringatan'])) }}"
      class="vsummary-item vsummary-item--warn {{ $filterKeterlambatan === 'peringatan' ? 'vsummary-item--active' : '' }}">
      <div class="vsummary-icon"><i class="fas fa-exclamation-triangle"></i></div>
      <div class="vsummary-meta">
        <div class="vsummary-label">Peringatan</div>
        <div class="vsummary-value" data-counter="{{ $dokumen24to72h ?? 0 }}" data-duration="1000">0</div>
    ...
    <a href="{{ route('documents.verifikasi.index', array_merge(request()->except('keterlambatan', 'page'), $filterKeterlambatan === 'terlambat' ? [] : ['keterlambatan' => 'terlambat'])) }}"
      class="vsummary-item vsummary-item--late {{ $filterKeterlambatan === 'terlambat' ? 'vsummary-item--active' : '' }}">
      <div class="vsummary-icon"><i class="fas fa-times-circle"></i></div>
      <div class="vsummary-meta">
        <div class="vsummary-label">Terlambat</div>
        <div class="vsummary-value" data-counter="{{ $dokumenMoreThan72h ?? 0 }}" data-duration="1000">0</div>
        <div class="vsummary-sub">&gt; 3 hari</div>
      </div>
    </a>
  </div>
  <!-- ===== END COMPACT SUMMARY METRICS ===== -->
  <script>
    (function() {
  ...
  <div class="table-dokumen" id="documentTableContainer">
            ...
            @foreach($selectedColumns as $col)
              @if($col !== 'status')
                @if($col === 'nomor_agenda')
                  ...
                  <th class="col-{{ $col }}" style="cursor: pointer; user-select: none;" onclick="toggleSort('nomor_agenda')">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                      <span>{{ $availableColumns[$col] ?? $col }}</span>
                      <div style="display: flex; flex-direction: column; gap: 2px;">
                        @php
                          $currentSort = request('sort', 'nomor_agenda');
                          $currentOrder = request('order', 'asc');
                          $isActiveNomor = $currentSort === 'nomor_agenda';
                        @endphp
                        <i class="fa-solid fa-caret-up"
                          style="font-size: 12px; color: {{ $isActiveNomor && $currentOrder === 'asc' ? '#1a4d3e' : '#cbd5e0' }}; line-height: 1; margin-bottom: -4px;"></i>
                        <i class="fa-solid fa-caret-down"
                          style="font-size: 12px; color: {{ $isActiveNomor && $currentOrder === 'desc' ? '#1a4d3e' : '#cbd5e0' }}; line-height: 1;"></i>
                      </div>
                    </div>
                  </th>
                @else
    ...
    <div id="returnToBidangModal" class="modal-overlay"
      ...
      <div class="modal-content"
        style="background: white; border-radius: 12px; padding: 24px; max-width: 450px; width: 90%; margin: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
        <div class="modal-header"
          style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
          ...
          <button type="button" onclick="closeReturnToBidangModal()"
            style="background: none; border: none; font-size: 24px; cursor: pointer; color: #6b7280; padding: 0; line-height: 1;">&times;</button>
        </div>
        <form id="returnToBidangForm" onsubmit="submitReturnToBidang(event)">
          ...
          <div class="modal-body">
            <div
              style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); padding: 16px; border-radius: 8px; margin-bottom: 16px; border-left: 4px solid #f59e0b;">
              <div style="display: flex; align-items: center; gap: 12px;">
                <i class="fa-solid fa-building" style="font-size: 24px; color: #b45309;"></i>
                <div>
                  <div style="font-size: 12px; color: #92400e; font-weight: 500; margin-bottom: 2px;">Dokumen akan
                    dikembalikan ke:</div>
            ...
            <div class="form-group" style="margin-bottom: 16px;">
              <label for="returnToBidangReason"
            ...
            <div style="background: #f3f4f6; padding: 12px; border-radius: 8px; font-size: 13px; color: #6b7280;">
              ...
              Dokumen yang dikembalikan akan dipindahkan ke halaman <strong>Pengembalian ke Bidang</strong> dan tidak lagi
              tampil di daftar utama.
            </div>
          </div>

          <div class="modal-footer"
            style="display: flex; gap: 12px; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
            <button type="button" onclick="closeReturnToBidangModal()"
    ...
    <!-- Modal Paraf Dokumen -->
    <div id="parafModal" class="modal-overlay"
      style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 9999; align-items: center; justify-content: center;">
      <div class="modal-content"
        style="background: white; border-radius: 12px; padding: 24px; max-width: 420px; width: 90%; margin: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
        <div class="modal-header"
          style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
          <h3 style="margin: 0; font-size: 18px; font-weight: 600; color: #1f2937;">
            <i class="fa-solid fa-stamp me-2" style="color: #083E40;"></i>
            Paraf Dokumen
          </h3>
          <button type="button" onclick="closeParafModal()"
            style="background: none; border: none; font-size: 24px; cursor: pointer; color: #6b7280; padding: 0; line-height: 1;">&times;</button>
        </div>
        <div class="modal-body">
          <div
            style="background: linear-gradient(135deg, #e6f7f1 0%, #d1f2e8 100%); padding: 16px; border-radius: 8px; margin-bottom: 16px; border-left: 4px solid #083E40;">
            <div style="display: flex; align-items: center; gap: 12px;">
              <i class="fa-solid fa-file-lines" style="font-size: 24px; color: #083E40;"></i>
              <div>
                <div style="font-size: 12px; color: #065f46; font-weight: 500; margin-bottom: 2px;">Dokumen yang akan
                  diparaf:</div>
                <div id="parafDocInfo" style="font-size: 16px; font-weight: 700; color: #064e3b;">-</div>
              </div>
            </div>
          </div>
          <div class="form-group" style="margin-bottom: 16px;">
            <label for="pemarafSelect"
              style="display: block; font-size: 14px; font-weight: 600; color: #374151; margin-bottom: 8px;">
              <i class="fa-solid fa-user-pen me-1"></i> Pilih Pemaraf <span style="color: #ef4444;">*</span>
            </label>
            <select id="pemarafSelect"
              style="width: 100%; padding: 12px 16px; border: 2px solid #d1d5db; border-radius: 8px; font-size: 14px; background: white; cursor: pointer; transition: border-color 0.2s; box-sizing: border-box;">
              <option value="">-- Pilih Pemaraf --</option>
              <option value="Ibu Yuni">Ibu Yuni</option>
              <option value="Sekar">Sekar</option>
            </select>
          </div>
          <div style="background: #f3f4f6; padding: 12px; border-radius: 8px; font-size: 13px; color: #6b7280;">
            <i class="fa-solid fa-info-circle me-2" style="color: #3b82f6;"></i>
            Setelah diparaf, dokumen akan ditandai sebagai sudah diverifikasi.
          </div>
        </div>
        <input type="hidden" id="parafDocId">
        <div class="modal-footer"
          style="display: flex; gap: 12px; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
          <button type="button" onclick="closeParafModal()"
            style="flex: 1; padding: 12px 16px; background: #e5e7eb; color: #374151; border: none; border-radius: 8px; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s;">Batal</button>
          <button type="button" id="parafSubmitBtn" onclick="submitParaf()"
            style="flex: 1; padding: 12px 16px; background: linear-gradient(135deg, #083E40 0%, #0a4f52 100%); color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px;">
            <i class="fa-solid fa-stamp"></i>
            <span>Paraf</span>
          </button>
        </div>
    ...
    <script>
      ...
      window.openParafModal = function (dokumenId, nomorAgenda) {
        document.getElementById('parafDocId').value = dokumenId;
        document.getElementById('parafDocInfo').textContent = nomorAgenda || 'Dokumen #' + dokumenId;
        document.getElementById('pemarafSelect').value = '';
        const modal = document.getElementById('parafModal');
        modal.style.display = 'flex';
        modal.onclick = function (e) {
          if (e.target === modal) closeParafModal();
        };
      };

      window.closeParafModal = function () {
        document.getElementById('parafModal').style.display = 'none';
      };

      window.submitParaf = function () {
        const dokumenId = document.getElementById('parafDocId').value;
        const pemaraf = document.getElementById('pemarafSelect').value;
        const submitBtn = document.getElementById('parafSubmitBtn');

        if (!pemaraf) {
          alert('Pilih pemaraf terlebih dahulu.');
          return;
        }
        ...
        fetch(`/documents/verifikasi/${dokumenId}/paraf`, {
                                method: 'POST',
                                headers: {
                                  'Content-Type': 'application/json',
                                  'X-CSRF-TOKEN': csrfToken,
                                ...
                                body: JSON.stringify({ pemaraf: pemaraf })
                              })
                              .then(response => response.json())
                              .then(data => {
                                if (data.success) {
                                  ...
                                  // Update paraf button to badge
                                  const container = document.getElementById('paraf-container-' + dokumenId);
                                  if (container) {
                                    container.innerHTML = '<span class="badge-paraf-done" style="width: 100%; justify-content: center; padding: 8px;"><i class="fa-solid fa-stamp"></i> Diparaf: ' + pemaraf + '</span>';
                                  }

                                  // Update table cells if columns are visible
                                  const row = document.querySelector(`tr[data-id="${dokumenId}"]`);
                                  if (row) {
                                    const parafCell = row.querySelector('.col-tanggal_paraf');
                                    if (parafCell) parafCell.textContent = data.tanggal_paraf || '-';
                                    const pemarafCell = row.querySelector('.col-pemaraf');
                                    if (pemarafCell) {
                                      pemarafCell.innerHTML = '<span class="badge-paraf-done"><i class="fa-solid fa-check-circle"></i> ' + pemaraf + '</span>';
                                    }
                                  }
                                } else {
                              ...
                              .catch(error => {
                                console.error('Error:', error);
                                alert('Terjadi kesalahan saat memparaf dokumen.');
                              })
                              .finally(() => {
                                submitBtn.disabled = false;
                                submitBtn.innerHTML = '<i class="fa-solid fa-stamp"></i> <span>Paraf</span>';
                              });
                            };
                              ...
                              fetch(`/documents/verifikasi/${docId}/detail`, {
                                ...
                                .then(data => {
                                  ...
                                  if (data.success && data.dokumen) {
                                    ...
                                    // Nomor MIRO (now consolidated via accessor in controller)
  ...
  <!-- Modal: Rejection Detail - Team Verifikasi -->
  ...
  <script>
    // Show rejection modal - Team Verifikasi
...
<style>
  ...
  #documentTableContainer .data-table tbody td.acn-active::before,
  body.is-fullscreen #documentTableContainer .data-table tbody td.acn-active::before,
  body.document-table-only-fullscreen #documentTableContainer .data-table tbody td.acn-active::before {
