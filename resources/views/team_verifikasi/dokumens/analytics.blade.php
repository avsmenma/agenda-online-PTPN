...
@section('content')
  ...
  <style>
    ...
    .btn-year-filter:hover {
      background: linear-gradient(135deg, #0a5a5c 0%, #083E40 100%);
  ...
  <div class="filter-year-section">
    <div class="filter-wrapper">
      <div class="filter-group">
        ...
        <button type="button" class="btn-year-filter" onclick="openYearFilterModal()">
          <i class="fa-solid fa-calendar-alt me-2"></i>
          <span id="yearFilterBtnText">
            @if($selectedYear)
              {{ $selectedYear }}
              @php
                $filterLabels = ['tanggal_spp' => 'Tgl SPP', 'tanggal_masuk' => 'Tgl Masuk', 'nomor_spp' => 'No SPP'];
              @endphp
              ({{ $filterLabels[$yearFilterType ?? 'tanggal_spp'] ?? 'Tgl SPP' }})
            @else
              Filter Tahun
            @endif
          </span>
        ...
        <input type="hidden" id="yearSelect" value="{{ $selectedYear ?? date('Y') }}">
  ...
  <script>
    ...
    let selectedYear = '{{ $selectedYear ?? date("Y") }}';
    ...
    function resetYearFilter() {
      ...
      document.querySelectorAll('.year-btn').forEach(btn => {
        btn.classList.remove('selected');
        if (btn.textContent.trim() === '{{ date("Y") }}') {
    ...
    function changeFilter() {
      ...
      const filterType = document.getElementById('yearFilterType').value;

      const url = new URL(window.location.href);
      url.searchParams.set('year', year);
      url.searchParams.set('year_filter_type', filterType);
      if (bagian) {
