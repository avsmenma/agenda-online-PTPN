...
{{-- ==== Modal Kustomisasi Kolom (disalin daftarPerpajakanTabulator.blade.php:76-184, self-contained per CLAUDE.md §7) ==== --}}
...
@push('styles')
    ...
    /* Modal Customization Styles (disalin daftarPerpajakanTabulator.blade.php:199-268, self-contained per CLAUDE.md §7) */
...
@push('scripts')
{{-- Kustomisasi Kolom — JS disalin dari daftarPerpajakanTabulator.blade.php:284-545
     (self-contained per CLAUDE.md §7), dengan appendActiveFilterInputs() ditulis
     ulang untuk membawa filter toolbar verifikasi (search/status/filter_dari) —
     nama field yang SAMA dengan perpajakan/akutansi, diverifikasi terhadap
     TeamVerifikasiController::buildVerifikasiQuery(). Verifikasi tak punya
     create/destroy — JS Hapus/Detail view operator TIDAK disalin. --}}
<script>
    ...
    function saveColumnCustomization() {
        ...
        filterForm.submit(); // GET → documents.verifikasi.index → reload view Tabulator dgn kolom baru.
    }

    // Bawa filter toolbar verifikasi (search/status/filter_dari) agar tak hilang saat reload GET.
