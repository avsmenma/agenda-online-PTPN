          ...
          <div class="flex items-center gap-3">
            <div class="text-right mr-4 hidden md:block">
              ...
              <p class="text-sm font-medium text-slate-700">
                {{ $dokumen->deadline_at ? \Carbon\Carbon::parse($dokumen->deadline_at)->format('d M Y') : '-' }}</p>
            </div>
            <a href="{{ $dashboardUrl ?? '/owner/dashboard' }}"
              class="inline-flex items-center justify-center px-4 py-2 border border-slate-300 shadow-sm text-sm font-medium rounded-lg text-slate-700 bg-white hover:bg-slate-50 hover:text-emerald-600 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
              ...
              <div class="{{ $cardBaseClass }} {{ $cardVisualClass }}">
                ...
                @if($isActive)
                  <div class="absolute -top-3 right-4">
                    <span
                      class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold leading-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-md">
                ...
                @if($isReturned)
                  <div class="absolute -top-3 right-4">
                    <span
                      class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold leading-4 bg-amber-500 text-white shadow-md">
                ...
                <div class="flex items-start justify-between mb-4">
                  <div class="flex items-center gap-4">
                    <div
                      class="h-12 w-12 rounded-2xl flex items-center justify-center {{ $iconBgClass }} transition-colors duration-300">
                      @if($isCompleted)
      ...
      <section>
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="lg:col-span-8 space-y-6">
            ...
            <div
              class="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
              <div
                class="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-white/5 rounded-full blur-3xl group-hover:bg-white/10 transition-colors duration-500">
              </div>
                  ...
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <p class="font-medium truncate">
                        {{ $dokumen->dibayarKepadas->first()?->nama_penerima ?? $dokumen->dibayar_kepada ?? '-' }}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                <div class="group col-span-full">
                  <p
                    class="text-xs text-slate-400 uppercase font-bold tracking-wider mb-1 group-hover:text-emerald-500 transition-colors">
                    Uraian SPP</p>
