...
<style>
  ...
  .card-stat.active {
    ...
    box-shadow: 0 12px 40px var(--primary-rgba-dark), 0 4px 16px rgba(64, 145, 108, 0.2);
  }
...
<div class="container-fluid">
  ...
  <div class="card-stats">
    ...
    <a href="{{ route('owner.rekapan-keterlambatan.role', array_merge([$roleCode], array_merge(request()->except(['filter_age', 'page']), ['filter_age' => $isCard1Active ? '' : '1']))) }}" 
       class="card-stat-link {{ $isCard1Active ? 'active' : '' }}">
      <div class="card-stat card-green {{ $isCard1Active ? 'active' : '' }}">
    ...
    <a href="{{ route('owner.rekapan-keterlambatan.role', array_merge([$roleCode], array_merge(request()->except(['filter_age', 'page']), ['filter_age' => $isCard2Active ? '' : '2']))) }}" 
       class="card-stat-link {{ $isCard2Active ? 'active' : '' }}">
      <div class="card-stat card-yellow {{ $isCard2Active ? 'active' : '' }}">
        <div class="card-stat-header">
    ...
    <a href="{{ route('owner.rekapan-keterlambatan.role', array_merge([$roleCode], array_merge(request()->except(['filter_age', 'page']), ['filter_age' => $isCard3Active ? '' : '3+']))) }}" 
       class="card-stat-link {{ $isCard3Active ? 'active' : '' }}">
      <div class="card-stat card-red {{ $isCard3Active ? 'active' : '' }}">
        <div class="card-stat-header">
  ...
  <div class="modern-filter-container">
    ...
    <div class="filter-panel" id="filterPanel" style="display: none;">
      <form method="GET" action="{{ route('owner.rekapan-keterlambatan.role', $roleCode) }}" id="filterForm" class="filter-form">
    ...
    @else
      <div class="card-view-container">
        @foreach($dokumens as $index => $dokumen)
          ...
          @endphp
          <div class="document-card" onclick="window.location.href='{{ route('owner.workflow', ['id' => $dokumen->id]) }}'" title="Klik untuk melihat detail workflow dokumen">
            ...
            <div class="card-info-row">
              ...
              <span class="age-badge age-{{ $ageColor }}">{{ $ageFormatted }}</span>
            </div>
            <div class="card-info-row">
              <i class="fas fa-info-circle"></i>
              <span>Status:</span>
              <span class="badge badge-processing">Belum Diproses</span>
            </div>
          </div>
        @endforeach
      </div>
