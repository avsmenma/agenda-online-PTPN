...
@section('content')
  ...
  <div class="form-container">
    ...
    @if(isset($hasPerpajakanData) && $hasPerpajakanData)
      ...
      <div class="section-title"
        style="background: linear-gradient(90deg, rgba(255, 193, 7, 0.1) 0%, transparent 100%); border-left-color: #ffc107;">
        <i class="fa-solid fa-file-invoice-dollar" style="color: #ffc107;"></i>
        Informasi Team Perpajakan
        <span
          style="background: #ffc107; color: white; padding: 4px 12px; border-radius: 20px; font-size: 10px; margin-left: 8px;">DATA
          DARI PERPAJAKAN</span>
      </div>
      ...
      <div class="info-grid-3" style="margin-top: 15px;">
        ...
        <div class="info-item">
          ...
          <div class="info-value">{{ $dokumen->dpp_pph ? 'Rp ' . number_format($dokumen->dpp_pph, 0, ',', '.') : '-' }}
          </div>
        </div>
        <div class="info-item">
          <div class="info-label">PPN Terhutang</div>
          <div class="info-value">
            {{ $dokumen->ppn_terhutang ? 'Rp ' . number_format($dokumen->ppn_terhutang, 0, ',', '.') : '-' }}</div>
        </div>
      </div>
    @endif
    ...
    @if(isset($hasAkutansiData) && $hasAkutansiData)
      <div class="divider"></div>
      <div class="section-title"
        style="background: linear-gradient(90deg, rgba(0, 123, 255, 0.1) 0%, transparent 100%); border-left-color: #007bff;">
        <i class="fa-solid fa-calculator" style="color: #007bff;"></i>
        Informasi Team Akutansi
        <span
          style="background: #007bff; color: white; padding: 4px 12px; border-radius: 20px; font-size: 10px; margin-left: 8px;">DATA
          DARI AKUTANSI</span>
      </div>
    ...
    <div class="required-note">
      ...
      <div class="required-note-text">
        ...
        <span>Isi minimal salah satu dari Tanggal Bayar atau Upload Bukti Pembayaran untuk mengubah status dokumen menjadi
          "Sudah Dibayar".</span>
      </div>
    </div>
